...
export default function SettingsScreen() {
  ...
  const option = <T extends string>(label: string, value: T, selected: boolean, onPress: (value: T) => void) => <Pressable key={value} onPress={() => onPress(value)} style={({ pressed }) => [styles.option, { borderBottomColor: palette.border, opacity: pressed ? 0.6 : 1 }]}><AppText style={styles.optionLabel}>{label}</AppText>{selected && <Check size={18} color={palette.accent} />}</Pressable>;
  ...
  return <Screen><View style={styles.header}><AppText style={styles.heading}>Settings</AppText></View><ScrollView contentContainerStyle={styles.content}>
    <AppText muted style={styles.section}>Appearance</AppText>{(["system", "light", "dark"] as ThemePreference[]).map((value) => option(value[0].toUpperCase() + value.slice(1), value, theme.preference === value, theme.setPreference))}
    <AppText muted style={styles.section}>Widgets</AppText><AppText muted style={styles.detail}>Each widget type keeps its own view, layout, filters, metadata, and appearance. Changes apply to both iOS and Android.</AppText>
