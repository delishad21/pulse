...
export default async function viewRoutes(app:FastifyInstance){
  app.get("/inbox",async(r,p)=>{const u=getUser(r);p.send(await getRepository().views.inbox(u.id));});
  app.get("/today",async(r,p)=>{const u=getUser(r);p.send(await getRepository().views.today(u.id,new Date(),u.timezone));});
  app.get("/upcoming",async(r,p)=>{const u=getUser(r);p.send(await getRepository().views.upcoming(u.id,new Date(),u.timezone));});
