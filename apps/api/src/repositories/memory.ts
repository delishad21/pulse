...
function validateRecurrence(value:string|null|undefined):void { if(!value)return;try{parseRecurrenceRule(value);}catch(error){throw Errors.Validation(error instanceof Error?error.message:"Invalid recurrence rule.");} }
...
export function createMemoryRepository(initialUserId?:string, additionalUserIds:string[]=[]):PulseRepository {
  ...
  const repo:PulseRepository={
    ...
    tasks:{
      ...
      bulkUpdate:async(userId,input)=>{ensureUser(userId);validateRecurrence(input.recurrenceRule);await validateRelations(userId,input);const out:Task[]=[];for(const id of input.ids){const t=getTask(userId,id);const s=mergedSchedule(t,input);if(input.title!==undefined)t.title=input.title;if(input.priority!==undefined)t.priority=parsePriority(input.priority);if(input.startAt!==undefined)t.startAt=s.startAt?new Date(s.startAt):null;if(input.endAt!==undefined||input.startAt===null)t.endAt=s.endAt?new Date(s.endAt):null;if(input.dueDate!==undefined){t.dueDate=s.due.date?new Date(`${s.due.date}T00:00:00Z`):null;if(input.dueAt===undefined)t.dueAt=null;}if(input.dueAt!==undefined){t.dueAt=s.due.at?new Date(s.due.at):null;if(input.dueDate===undefined)t.dueDate=null;}if(input.recurrenceRule!==undefined)t.recurrenceRule=input.recurrenceRule;if(input.projectId!==undefined)t.projectId=input.projectId;if(input.addTagIds)for(const tagId of input.addTagIds)if(!t.tagIds.includes(tagId))t.tagIds.push(tagId);if(input.removeTagIds)t.tagIds=t.tagIds.filter((x)=>!input.removeTagIds!.includes(x));t.revision+=1;t.updatedAt=new Date();out.push(toTaskRecord(t));}return out;},
