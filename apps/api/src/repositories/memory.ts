...
interface DbOperation { id: string; userId: string; taskId: string | null; kind: string; payload: unknown; undoneAt: Date | null; createdAt: Date; }
...
export function createMemoryRepository(initialUserId?: string, additionalUserIds: string[] = []): PulseRepository {
  ...
  const repo: PulseRepository = {
    ...
    operations: {
      record: async(userId,kind,payload,taskId)=>{ensureUser(userId);const o:DbOperation={id:cuid(),userId,taskId:taskId??null,kind,payload,undoneAt:null,createdAt:new Date()};operations.set(o.id,o);return serializeOperation(o as never);},
      list: async(userId)=>{ensureUser(userId);return [...operations.values()].filter((o)=>o.userId===userId).sort((a,b)=>b.createdAt.getTime()-a.createdAt.getTime()).slice(0,3).map((o)=>serializeOperation(o as never));},
      undoLast: async(userId)=>{ensureUser(userId);const o=[...operations.values()].filter((o)=>o.userId===userId&&!o.undoneAt).sort((a,b)=>b.createdAt.getTime()-a.createdAt.getTime())[0];if(!o)throw Errors.NotFound("Operation");return repo.operations.undo(userId,o.id);},
      undo: async(userId,id)=>{ensureUser(userId);const o=operations.get(id);if(!o||o.userId!==userId)throw Errors.NotFound("Operation");if(o.undoneAt)throw Errors.Conflict("Operation already undone.");const payload=o.payload as Record<string,unknown>;if(o.kind==="TASK_CREATE"){const taskId=payload.taskId as string;const t=getTask(userId,taskId,true);t.deletedAt=new Date();t.revision+=1;}else if(["TASK_UPDATE","TASK_DELETE","TASK_COMPLETE","TASK_REOPEN"].includes(o.kind)){await repo.tasks.restoreSnapshot(userId,payload as unknown as TaskSnapshot);}else if(["TASK_BULK_UPDATE","TASK_BULK_COMPLETE","TASK_BULK_DELETE","TASK_BULK_MOVE"].includes(o.kind)){for(const snapshot of (payload.snapshots as TaskSnapshot[]??[]))await repo.tasks.restoreSnapshot(userId,snapshot);}else throw Errors.Validation(`Undo not supported for operation kind: ${o.kind}`);o.undoneAt=new Date();return serializeOperation(o as never);},
    },
  };
  return repo;
}
