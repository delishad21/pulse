...
export const prismaRepository: PulseRepository = {
  ...
  tasks: {
    ...
    create: async(userId,input)=>{validateRecurrence(input.recurrenceRule);const rel=await relationTargets(userId,input);const schedule=normalizeTaskSchedule(input);let created=await prisma.task.create({data:{userId,title:input.title,description:input.description??null,priority:priority(input.priority??"none"),dueDate:schedule.due.date?new Date(`${schedule.due.date}T00:00:00Z`):null,dueAt:schedule.due.at?new Date(schedule.due.at):null,reminderAt:schedule.reminderAt?new Date(schedule.reminderAt):null,recurrenceRule:input.recurrenceRule??null,projectId:rel.projectId??null,sectionId:rel.sectionId??null,parentTaskId:input.parentTaskId??null,sortOrder:input.sortOrder??0},include:taskInclude});if(input.tagIds?.length){await prisma.taskTag.createMany({data:input.tagIds.map((tagId)=>({taskId:created.id,tagId}))});created=(await prisma.task.findUnique({where:{id:created.id},include:taskInclude}))!;}return serializeTask(created);},
  ...
  operations: {
    ...
    undo: async(userId,id)=>{const o=await prisma.operation.findFirst({where:{id,userId}});found(o,"Operation");if(o.undoneAt)throw Errors.Conflict("Operation already undone.");const payload=o.payload as Record<string,unknown>;let redoSnapshots:TaskSnapshot[]=[];if(o.kind==="TASK_CREATE"){const taskId=payload.taskId as string;redoSnapshots=[await snapshot(userId,taskId)];await ensureTask(userId,taskId,true);await prisma.task.update({where:{id:taskId},data:{deletedAt:new Date(),revision:{increment:1}}});}else if(["TASK_UPDATE","TASK_DELETE","TASK_COMPLETE","TASK_REOPEN"].includes(o.kind)){const original=payload as unknown as TaskSnapshot;redoSnapshots=[await snapshot(userId,original.taskId)];await restoreSnapshot(userId,original);}else if(["TASK_BULK_UPDATE","TASK_BULK_COMPLETE","TASK_BULK_DELETE","TASK_BULK_MOVE"].includes(o.kind)){const originals=((payload.snapshots as TaskSnapshot[])??[]);redoSnapshots=await Promise.all(originals.map((item)=>snapshot(userId,item.taskId)));for(const item of originals)await restoreSnapshot(userId,item);}else throw Errors.Validation(`Undo not supported for operation kind: ${o.kind}`);const updated=await prisma.operation.update({where:{id},data:{payload:{...payload,redoSnapshots} as never,undoneAt:new Date()}});return serializeOperation(updated);},
