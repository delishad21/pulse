...
import { ColorPicker, themeColors } from "./color-picker";
...
export function LabelManager() {
  ...
  const close = () => { setOpen(false); setName(""); setColor(themeColors[0]!); };
    ...
    <div>
      ...
      <div className="divide-y divide-stroke">
        {tags?.map((tag) => (
          <div key={tag.id} className="group flex min-h-11 items-center gap-3 px-1 py-2">
            <Tag className="size-4" style={{ color: tag.color ?? themeColors[0] }} />
            <span className="min-w-0 flex-1 truncate text-sm font-medium text-ink">{tag.name}</span>
            <ColorPicker ariaLabel={`Color for ${tag.name}`} value={tag.color ?? themeColors[0]!} onChange={(value) => updateTag.mutate({ id: tag.id, input: { color: value } })} />
      ...
      {open ? (
        <div className="fixed inset-0 z-[90] flex items-center justify-center bg-black/40 px-4" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}>
          <form onSubmit={submit} role="dialog" aria-modal="true" aria-label="Add label" className="w-full max-w-[480px] rounded-xl border border-stroke bg-surface shadow-float">
            ...
            <div className="space-y-4 p-4">
              ...
              <div><p className="mb-2 text-xs font-bold text-ink">Color</p><div className="grid grid-cols-8 gap-2 sm:grid-cols-10">
                {themeColors.map((value) => <button key={value} type="button" aria-label={`Choose color ${value}`} onClick={() => setColor(value)} className={cn("size-8 rounded-lg border-2 transition", color === value ? "border-ink scale-105" : "border-transparent")} style={{ backgroundColor: value }} />)}
              </div></div>
              <div className="flex items-center justify-between rounded-lg border border-stroke px-3 py-2 text-sm font-medium text-ink"><span>Custom color</span><ColorPicker ariaLabel="Custom label color" value={color} onChange={setColor} showValue /></div>
            </div>
