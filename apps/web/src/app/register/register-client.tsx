...
export function RegisterClient() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    const result = await signIn("credentials", { username: form.username, password: form.password, redirect: false });
  ...
  return <main className="relative flex min-h-screen items-center justify-center bg-canvas p-5">
    ...
    <div className="w-full max-w-[420px]">
      <div className="mb-8 flex justify-center"><div className="flex items-center gap-3"><Image src="/pulse-logo.png" alt="Pulse" width={51} height={51} priority className="size-[51px] object-contain" /><div><div className="text-2xl font-extrabold tracking-tight text-ink">Pulse</div><div className="text-xs font-medium text-muted">Create your workspace</div></div></div></div>
      <section className="rounded-2xl border border-stroke bg-surface p-7 shadow-float sm:p-8">
        ...
        <form onSubmit={submit} className="mt-6 space-y-4">
          ...
          <label className="block"><span className="mb-1.5 block text-sm font-semibold text-ink">Username</span><input type="text" required autoComplete="username" value={form.username} onChange={(e)=>setForm({...form,username:e.target.value})} className="h-11 w-full rounded-lg border border-stroke bg-surface-subtle px-3 text-sm text-ink outline-none focus:border-primary" /></label>
