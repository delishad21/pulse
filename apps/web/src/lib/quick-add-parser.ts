...
function tokenise(source:string,candidates:Candidate[]):Array<Candidate&{id:string}>{const counts=new Map<string,number>();let previousEnd=0;return [...candidates].filter((candidate)=>Number.isInteger(candidate.start)&&Number.isInteger(candidate.end)&&candidate.start>=0&&candidate.end>candidate.start&&candidate.end<=source.length).sort((a,b)=>a.start-b.start||b.end-a.end).flatMap((candidate)=>{if(candidate.start<previousEnd)return[];previousEnd=candidate.end;const text=source.slice(candidate.start,candidate.end);const base=`${candidate.type}:${text.toLowerCase()}`;const n=counts.get(base)??0;counts.set(base,n+1);return[{...candidate,text,id:`${base}:${n}`}];});}
...
function dateCandidate(text:string,now:Date,blocked:Candidate[]):Candidate|undefined{
  const tries:Array<()=>Candidate|undefined>=[
    ...
    ()=>{const m=text.match(new RegExp(`\\b(?:this\\s+)?(${WEEKDAY_WORD})\\b`,"i"));if(!m||m.index===undefined)return;const info=WEEKDAYS[m[1].toLowerCase()];return info?{type:"date",start:m.index,end:m.index+m[0].length,text:m[0],label:m[0],value:localDate(nextWeekday(now,info.day,false))}:undefined;},
...
export function parseQuickAddDetailed(text:string,context:QuickAddContext={}):QuickAddDetailed{
  ...
  const all=tokenise(text,candidates);const ignored=context.ignoredTokenIds??new Set<string>();const active=all.filter((t)=>!ignored.has(t.id));
