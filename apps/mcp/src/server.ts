...
const taskFields=z.object({
  title:z.string().trim().min(1),description:z.string().nullable().optional(),location:z.string().max(500).nullable().optional(),priority:prioritySchema.optional(),
...
export function createPulseMcpServer(api:PulseMcpApi):McpServer{
  ...
  server.registerTool("create_task",{description:"Create a fully structured Pulse task. Interpret the user's natural language yourself before calling this tool. Use startAt/endAt for a scheduled time window, dueDate/dueAt only for a deadline, projectId for a project, tagIds for labels, location for the task place, priority none/low/medium/high/urgent, reminders for zero or more reminder instants, and recurrenceRule for recurring tasks. Date-only deadlines must use dueDate.",inputSchema:createTaskSchema},async(input)=>run(async()=>({task:await api.createTask(input as CreateTaskInput)})));
